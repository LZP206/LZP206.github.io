...
int* func()
{
  // 在堆区开辟数据：new+数据类型，返回一个地址， 用指针接收
	int* a = new int(10);   
  // 10这个数据人为开辟在堆区，
  // 指针a存放数据10的地址，指针a为局部变量存放在栈区
   // 另一种表达：
  // int * a;
  // a = new int;   
  // *a = 10;
...
// 这里对应1.3节指针做返回值时 用new操作符在堆区开辟内存
...
```C++
//发现是引用，编译器自动转换为 int* const ref = &a;
void func(int& ref){
...
int main(){
    ...
    //编译器自动转换为 int* const ref = &a; 指针常量是指针指向不可改，也说明为什么引用不可更改
	int& ref = a; 
...
int main() {
  ...
  // 得50，说明如果main函数中实参先形参传递了值，那就不使用形参默认值
...
int main() {
  ...
  p.showPerson();  // 输出 = Name:李四	Password:255333
...
{
	Person p;
  // p是栈区上的数据，test01执行完毕后会释放这个对象，从而自动调用对象的析构函数
}
...
void test02() {
  ...
  //注意2：不能利用拷贝构造函数初始化匿名对象，即 Person(p2); 编译器认为是对象声明,相当于Person p2 会造成重定义错误
...
//2. 值传递的方式给函数参数传值   （形参p1和实参p的地址不一样）
...
//3. 以值方式返回局部对象    （形参p1和实参p的地址不一样，linux、mac下相同？不会调用拷贝构造函数）
Person doWork2(){
	Person p1;
	cout << (int *)&p1 << endl;
	return p1;
...
{
	Person p1(18);
	//如果不自己写拷贝构造，编译器会自动添加拷贝构造，并且做浅拷贝操作
	Person p2(p1);
...
深拷贝：在堆区重新申请空间，需要利用深拷贝进行拷贝操作，防止浅拷贝带来的问题：堆区的内存被重复释放
...
public:
    ...
    //等价于 m_height = new int；*m_height = *p.m_height；
...
void test01() {
  ...
  person.ShowPerson();
	...
	person.MyFunc(); //常对象不能调用普通成员函数，因为普通成员可以修改成员属性，而常对象不能修改，矛盾
